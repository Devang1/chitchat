...
import GroupCreator from"../create-channel/index"
...
function ChatContainer() {
  ...
  }, [selectedChatMessages,selectedChatData,selectedChatType,userinfo,setselectedChatMessages]);
      ...
      <main className="flex-1 flex flex-col bg-gray-900/80 backdrop-blur-lg border-l border-gray-700">
        <header className="p-4 font-bold text-lg border-b border-gray-700 text-cyan-400 flex justify-between px-10 items-center">
          <div className="flex gap-5 items-center">
            ...
            />
            <h2>{`${selectedChatData.firstname} ${
              selectedChatData.lastname != null ? selectedChatData.lastname : ""
            }`}</h2>
          </div>
          <i
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center" >
          <i className="fa-solid fa-xmark text-4xl absolute top-15 right-15 cursor-pointer text-cyan-600" onClick={() => setimgclick(null)}></i>
          <i className="fa-solid fa-download text-3xl absolute top-15 right-30 cursor-pointer text-cyan-600" onClick={downloadImage}></i>
          ...
          {selectedChatMessages.length===0?(<div className='flex items-center justify-center h-[100%]'><h1 className='text-3xl text-cyan-600 font-medium'>Be first to send message</h1></div>):(selectedChatMessages.map(
            (msg, index) => (
              msg.message_type != "file" ?(
                ...
                <div
                  key={index}
                  className={`p-2  my-2 max-w-[60vw] md:max-w-[19vw] h-auto flex items-center justify-center  ${
                    msg.sender === userinfo.id
                      ? "ml-auto bg-cyan-600"
                      : "bg-gray-700"
                  } rounded-4xl shadow-md shadow-cyan-400/30`}
    ...
    <div className="flex h-screen bg-black text-white w-[80vw] ">
      <main className="flex-1 flex flex-col bg-gray-900/80 backdrop-blur-lg border-l border-gray-700">
        ...
        >
          {imgclick && (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex justify-center items-center" >
          <i className="fa-solid fa-xmark text-4xl absolute top-15 right-15 cursor-pointer text-cyan-600 drop-shadow-md" onClick={() => setimgclick(null)}></i>
          <i className="fa-solid fa-download text-3xl absolute top-15 right-30 cursor-pointer text-cyan-600 drop-shadow-md" onClick={downloadImage}></i>
          <img src={imgclick} alt="Preview" className="max-w-full max-h-full"  />
        </div>
      )}
